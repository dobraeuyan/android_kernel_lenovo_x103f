...
static struct sensors_classdev sensors_cdev = {
	...
	.max_range = "19.6", /* 2g */
	.resolution = "0.153", /* Resolution for ~8bit effective data after shift ((2*9.8*2)/256) */
...
//printk("------------acceld->accel_drdy =%d acceld->delay=%d\n",acceld->accel_drdy,atomic_read(&acceld->delay));
	//if (acceld->accel_drdy == 0)
	//	queue_delayed_work(acceld->accel_workqueue, &acceld->accel_work,
				    ...
				    unsigned int enable)
{
	...
	if (enable) {
		if (pre_enable == 0) {
			...
			acceld->kionix_accel_update_odr(acceld, atomic_read(&acceld->delay)/2);
			if (acceld->accel_drdy==0) {
                       		ktime = ktime_set(0,
					...
					unsigned int delay_ms)
{
	...
	if (delay_ms < KIONIX_ACCEL_MIN_DELAY)
		delay_ms = KIONIX_ACCEL_MIN_DELAY/2;
	if (delay_ms > KIONIX_ACCEL_MAX_DELAY)
		delay_ms = KIONIX_ACCEL_MAX_DELAY/2;
	atomic_set(&acceld->delay, (unsigned int)delay_ms/2);
		...
		int axis, int apply_now)
{
	...
	acceld->accel_cali[acceld->axis_map_z] = -sum_z + 1024;//16384
			      ...
			      const struct i2c_device_id *id)
{
	...
	atomic_set(&acceld->accel_enable_resume, 0);
	atomic_set(&acceld->delay, accel_pdata->poll_interval/2);
...
#ifdef CONFIG_GET_HARDWARE_INFO
	register_hardware_info(GSENSOR, kionix_accel_match_table[0].compatible);
#endif
